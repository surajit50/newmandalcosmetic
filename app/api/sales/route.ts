...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ===============================
    // PRODUCT LOOKUP (for validation & unit fallback)
    // ===============================
    ...
    // Validate stock before any creation
    ...
    const lineItems = body.items.map((item: any) => {
      ...
      const taxable = inclusive / (1 + item.gstRate / 100);
      const gstAmount = inclusive - taxable;

      // 🔑 Ensure unit is present – use sent unit, then product unit, then default
      const unit = item.unit || product?.unit || "pcs";

      return {
        ...item,
        unit,                  // ✅ unit will be stored in SaleItem
        gstAmount,
        total: inclusive,
      };
    });
    ...
    const paymentMode =
      paymentModeMap[String(body.paymentMode || "CASH").toUpperCase()] || "CASH";
    ...
    const sale = await prisma.sale.create({
      data: {
        ...
        paidAmount,
        dueAmount,
        paymentStatus: getPaymentStatus(dueAmount, paidAmount) as
          | "PAID"
          | "PARTIAL"
          | "DUE",
        paymentMode,
        ...
        items: {
          create: lineItems.map((item: any) => ({
            ...
            total: item.total,
            unit: item.unit,                        // 🟢 unit now stored
          })),
        },
      },
      include: { items: true },
    ...
    if (paidAmount > 0) {
      ...
      try {
        const lastEntries = await prisma.cashbook.findMany({
          orderBy: { createdAt: "desc" },
          take: 1,
        });
        previousBalance = lastEntries[0]?.newBalance || 0;
      } catch (error) {
  ...
  try {
    ...
    const [sales, total] = await Promise.all([
      prisma.sale.findMany({
        where,
        include: { items: true },           // items now include unit
        orderBy: { createdAt: "desc" },
        skip,
        take: limit,
      }),
